...
*
*/
class Grabdb extends MY_Model {
    function __construct()
    {
        parent::__construct();
    }

    //=== GET PAGING grab LOGS
    function getpaging_logs($where = '', $data = [], $order = "", $page = 1)
    {
        ...
        $sql =
            "SELECT * FROM $this->tbl_grab_logs
    ...
    //=== END GET PAGING grab LOGS

    //=== START grab LOGS
    function insert_log($data)
    {
        $result = $this->db->insert($this->tbl_grab_logs, $data);
        if ($result) {
    ...
    {
        $this->db->where('sicpt_id', $id);
        return $this->db->update($this->tbl_grab_logs, $data);
    }
    //=== END
    ...
    {
        $sql = "DELETE FROM {$this->tbl_grab_logs} WHERE `created_date` < ?";
