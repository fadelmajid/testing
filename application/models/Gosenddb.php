...
*
*/
class Gosenddb extends MY_Model {
    function __construct()
    {
        parent::__construct();
    }

    //=== GET PAGING GOSEND LOGS
    function getpaging_logs($where = '', $data = [], $order = "", $page = 1)
    {
        ...
        $sql =
            "SELECT * FROM $this->tbl_gosend_logs
    ...
    //=== END GET PAGING GOSEND LOGS

    //=== START GOSEND LOGS
    function insert_log($data)
    {
        $result = $this->db->insert($this->tbl_gosend_logs, $data);
        if ($result) {
    ...
    {
        $this->db->where('gosend_id', $id);
        return $this->db->update($this->tbl_gosend_logs, $data);
    }
    //=== END
    ...
    {
        $sql = "DELETE FROM {$this->tbl_gosend_logs} WHERE `created_date` < ?";
