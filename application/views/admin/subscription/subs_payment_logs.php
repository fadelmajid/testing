...
<div class="row wrapper border-bottom white-bg page-heading">
    <div class="col-12">
        <h2>Subscription Payment Logs</h2>
    </div>
</div>
                        ...
                        <tbody>
                            <?php
                                ...
                                foreach ($all_data as $value){
                                    echo '
                                        <tr>
                                            <td>'. $value->splog_id .'</td>
                                            <td>'. $value->splog_type .'</td>
                                            <td>'. show_date($value->created_date, true) .'</td>
                                            <td>'. $value->splog_endpoint .'</td>
                                            <td>
                                                <a href="#" data-clipboard-target="#header'.$value->splog_id.'" id="splog">'. substr($value->splog_header, 0, 50) .'</a>
                                                <textarea id="header'. $value->splog_id .'" style="opacity: .01;height:0;position:absolute;z-index: -1;">'. $value->splog_header .'</textarea>
                                            </td>
                                            <td>
                                                <a href="#" data-clipboard-target="#request'.$value->splog_id.'" id="splog">'. substr($value->splog_request, 0, 50) .'</a>
                                                <textarea id="request'. $value->splog_id .'" style="opacity: .01;height:0;position:absolute;z-index: -1;">'. $value->splog_request .'</textarea></td>
                                            <td>
                                                <a href="#" data-clipboard-target="#response'.$value->splog_id.'" id="splog">'. substr($value->splog_response, 0, 50) .'
                                                <textarea id="response'. $value->splog_id .'" style="opacity: .01;height:0;position:absolute;z-index: -1;">'. $value->splog_response .'</textarea>
                                            </td>
                                        </tr>
                                    ';
                                }
