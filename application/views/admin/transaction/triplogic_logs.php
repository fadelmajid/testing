                        ...
                        <tbody>
                            <?php
                                ...
                                foreach ($all_data as $value){
                                    echo '
                                        <tr>
                                            <td>'. $value->triplogic_id .'</td>
                                            <td>'. $value->triplogic_type .'</td>
                                            <td>'. show_date($value->created_date, true) .'</td>
                                            <td>'. $value->triplogic_endpoint .'</td>
                                            <td>
                                                <a href="#" data-clipboard-target="#header'.$value->triplogic_id.'" id="triplogic">'. substr($value->triplogic_header, 0, 50) .'</a>
                                                <textarea id="header'. $value->triplogic_id .'" style="opacity: .01;height:0;position:absolute;z-index: -1;">'. $value->triplogic_header .'</textarea>
                                            </td>
                                            <td>
                                                <a href="#" data-clipboard-target="#request'.$value->triplogic_id.'" id="triplogic">'. substr($value->triplogic_request, 0, 50) .'</a>
                                                <textarea id="request'. $value->triplogic_id .'" style="opacity: .01;height:0;position:absolute;z-index: -1;">'. $value->triplogic_request .'</textarea></td>
                                            <td>
                                                <a href="#" data-clipboard-target="#response'.$value->triplogic_id.'" id="triplogic">'. substr($value->triplogic_response, 0, 50) .'
                                                <textarea id="response'. $value->triplogic_id .'" style="opacity: .01;height:0;position:absolute;z-index: -1;">'. $value->triplogic_response .'</textarea>
                                            </td>
                                        </tr>
                                    ';
                                }
