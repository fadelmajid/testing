                            ...
                            <?php
                                ...
                                foreach ($all_data as $value){
                                    echo '
                                        <tr>
                                            <td>'. $value->grab_id .'</td>
                                            <td>'. $value->grab_type .'</td>
                                            <td>'. show_date($value->created_date, true) .'</td>
                                            <td>'. $value->grab_endpoint .'</td>
                                            <td>
                                                <a href="#" data-clipboard-target="#header'.$value->grab_id.'" id="grab">'. substr($value->grab_header, 0, 50) .'</a>
                                                <textarea id="header'. $value->grab_id .'" style="opacity: .01;height:0;position:absolute;z-index: -1;">'. $value->grab_header .'</textarea>
                                            </td>
                                            <td>
                                                <a href="#" data-clipboard-target="#request'.$value->grab_id.'" id="grab">'. substr($value->grab_request, 0, 50) .'</a>
                                                <textarea id="request'. $value->grab_id .'" style="opacity: .01;height:0;position:absolute;z-index: -1;">'. $value->grab_request .'</textarea></td>
                                            <td>
                                                <a href="#" data-clipboard-target="#response'.$value->grab_id.'" id="grab">'. substr($value->grab_response, 0, 50) .'
                                                <textarea id="response'. $value->grab_id .'" style="opacity: .01;height:0;position:absolute;z-index: -1;">'. $value->grab_response .'</textarea>
                                            </td>
                                        </tr>
                                    ';
                                }
