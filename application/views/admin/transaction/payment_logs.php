                        ...
                        <tbody>
                            <?php
                                ...
                                foreach ($all_data as $value){
                                    echo '
                                        <tr>
                                            <td>'. $value->pylog_id .'</td>
                                            <td>'. $value->pylog_type .'</td>
                                            <td>'. show_date($value->created_date, true) .'</td>
                                            <td>'. $value->pylog_endpoint .'</td>
                                            <td>
                                                <a href="#" data-clipboard-target="#header'.$value->pylog_id.'" id="pylog">'. substr($value->pylog_header, 0, 50) .'</a>
                                                <textarea id="header'. $value->pylog_id .'" style="opacity: .01;height:0;position:absolute;z-index: -1;">'. $value->pylog_header .'</textarea>
                                            </td>
                                            <td>
                                                <a href="#" data-clipboard-target="#request'.$value->pylog_id.'" id="pylog">'. substr($value->pylog_request, 0, 50) .'</a>
                                                <textarea id="request'. $value->pylog_id .'" style="opacity: .01;height:0;position:absolute;z-index: -1;">'. $value->pylog_request .'</textarea></td>
                                            <td>
                                                <a href="#" data-clipboard-target="#response'.$value->pylog_id.'" id="pylog">'. substr($value->pylog_response, 0, 50) .'
                                                <textarea id="response'. $value->pylog_id .'" style="opacity: .01;height:0;position:absolute;z-index: -1;">'. $value->pylog_response .'</textarea>
                                            </td>
                                        </tr>
                                    ';
                                }
